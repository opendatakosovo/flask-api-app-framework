...
from app import user_mongo_utils, content_mongo_utils
...
def my_articles(article_action):
	...
	return render_template('mod_article/article_management.html', articles = articles, article_action=article_action, message=message)
...
def new_article():
	...
	if request.method == "GET":
		...
		if action == "save":
			content_mongo_utils.add_article({"content":content,"visible":True, "category": category, "title": title, "slug": slugify(title), "user":current_user.id, "published":False, "published_date":datetime.now(), "author_slug":current_user.user_slug, "author_name":current_user.name,"author_lastname": current_user.lastname})
